...
#if !defined(AI_NETWORK_FAILURE_INPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_in_1[AI_NETWORK_FAILURE_IN_1_SIZE_BYTES];
ai_i8* data_ins[AI_NETWORK_FAILURE_IN_NUM] = {
data_in_1
};
#else
ai_i8* data_ins[AI_NETWORK_FAILURE_IN_NUM] = {
...
#if !defined(AI_NETWORK_FAILURE_OUTPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_out_1[AI_NETWORK_FAILURE_OUT_1_SIZE_BYTES];
ai_i8* data_outs[AI_NETWORK_FAILURE_OUT_NUM] = {
data_out_1
};
#else
ai_i8* data_outs[AI_NETWORK_FAILURE_OUT_NUM] = {
...
AI_ALIGNED(32)
static uint8_t pool0[AI_NETWORK_FAILURE_DATA_ACTIVATION_1_SIZE];

ai_handle data_activations0[] = {pool0};

/* AI objects ----------------------------------------------------------------*/

static ai_handle network_failure = AI_HANDLE_NULL;
...
static int ai_boostrap(ai_handle *act_addr)
{
  ...
  err = ai_network_failure_create_and_init(&network_failure, act_addr, NULL);
  if (err.type != AI_ERROR_NONE) {
    ai_log_err(err, "ai_network_failure_create_and_init");
    return -1;
  }

  ai_input = ai_network_failure_inputs_get(network_failure, NULL);
  ai_output = ai_network_failure_outputs_get(network_failure, NULL);

#if defined(AI_NETWORK_FAILURE_INPUTS_IN_ACTIVATIONS)
  /*  In the case where "--allocate-inputs" option is used, memory buffer can be
   *  used from the activations buffer. This is not mandatory.
   */
  for (int idx=0; idx < AI_NETWORK_FAILURE_IN_NUM; idx++) {
   ...
   */
  for (int idx=0; idx < AI_NETWORK_FAILURE_OUT_NUM; idx++) {
...
static int ai_run(void)
{
  ...
  batch = ai_network_failure_run(network_failure, ai_input, ai_output);
  if (batch != 1) {
    ai_log_err(ai_network_failure_get_error(network_failure),
        "ai_network_failure_run");
    return -1;
  }
